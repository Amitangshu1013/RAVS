"""
This code provides inference using GFNet with eval mode 1. The training dataset loading is commented.
The attack code by TorchAttacks is augmented within the generate_logits function. 
"""
...
#PyTorch weights for Surrogate Models
import torchvision
    ...
    if args.eval_mode > 0:
        ...
        val_loader = torch.utils.data.DataLoader(
            datasets.ImageFolder(valdir, transforms.Compose([
                ...
                transforms.ToTensor(),
                normalize])),
            batch_size=128, shuffle=False, num_workers=8, pin_memory=False)
...
def generate_logits(model_prime, model, fc, memory, policy, dataloader, maximum_length, prime_size, patch_size, model_arch):
    ...
    net_1 = torchvision.models.resnet34(weights=ResNet34_Weights.DEFAULT)
    #net_1 = torchvision.models.resnet50(weights=ResNet50_Weights.IMAGENET1K_V1)
    ...
    for i, (x, target) in enumerate(dataloader):
        ...
        #breakpoint() # input_var = 224 224
        # Create Attack before get_prime
        with torch.enable_grad():
            input_var = atk(input_var, target_var)
        #breakpoint() # input_var = 224 224 (dimensions)
        input_prime = get_prime(input_var, prime_size, model_configurations[model_arch]['prime_interpolation'])
        #breakpoint() # input_prime = 96 or 128 (dimensions) depending on chosen pre-trained model
        with torch.no_grad():
